...
export default function useSellerIssueReports() {
	...
	useEffect(() => {
		async function fetchIssues() {
			...
			try {
				const bundlesResponse = await fetch(`${API_BASE_URL}/sellers/me/bundles`, {
					method: "GET",
					headers: {
				...
				if (!bundlesResponse.ok) {
					const details = await bundlesResponse.json().catch(() => null);
					throw new Error(details?.detail || "Failed to fetch bundles");
				}
				...
				for (const bundle of bundles) {
					...
					const uncollectedExpiredReservations = reservations.filter((reservation) => {
						const collectedAt = toDate(reservation.collected_at);
					...
					for (const reservation of uncollectedExpiredReservations) {
						derivedIssues.push({
							...
							status: "open",
							created_at: bundle.window_end || reservation.reserved_at,
							description:
